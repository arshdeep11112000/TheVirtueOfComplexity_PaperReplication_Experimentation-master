...
try:
    ...
    class _DummyProgressBar:
        """Minimal replacement for progressbar.ProgressBar."""
    ...
    class _DummyProgressbarModule:
        ProgressBar = _DummyProgressBar
    ...
    progressbar = _DummyProgressbarModule()
...
class InstrumentedPCA(BaseEstimator):
    ...
    l1_ratio : scalar
        ...
        Set to 0 for pure ridge-style updates (no L1 / no coordinate descent).
    ...
    def __init__(self, n_factors=1, intercept=False, max_iter=10000,
                 iter_tol=10e-6, alpha=0., l1_ratio=0., n_jobs=1,
                 backend="loky"):
...
def _Gamma_fit_panel(F_New, X, y, indices, PSF, L, Ktilde, alpha, l1_ratio,
    ...
    if alpha > ridgeless_alpha_tol and l1_ratio > 0:
        print(
            "Gamma fit solver: ElasticNet (coordinate descent), "
    ...
    # L2-only case: ridge unless alpha is effectively zero.
    elif alpha > ridgeless_alpha_tol:
        print(
            "Gamma fit solver: Ridge, "
            f"alpha={alpha}, l1_ratio={l1_ratio}, ridge_solver={ridge_solver}"
        )
        gamma = _ridge_fit(ZkF, y, alpha=alpha, solver=ridge_solver)

    # alpha ~ 0: ridgeless minimum-norm fit
    else:
        print(
            "Gamma fit solver: Ridgeless (minimum-norm), "
            f"alpha={alpha}, l1_ratio={l1_ratio}, rcond={ridgeless_rcond}"
        )
        gamma = _ridgeless_fit(ZkF, y, rcond=ridgeless_rcond)
...
def _ridge_fit(X, y, alpha, solver="auto"):
    ...
    print("Ridge regression with alpha =", alpha, "and solver =", solver)
    if X.shape[0] != y.shape[0]:
        raise ValueError("X and y have incompatible shapes for ridge fit.")

    mod = Ridge(alpha=float(alpha), fit_intercept=False, solver=solver)
    mod.fit(X, y)
    return np.asarray(mod.coef_, dtype=np.float64).reshape(-1)


def _ridgeless_fit(X, y, rcond=None):
    """Return the minimum-norm ridgeless solution for linear regression."""
    X = np.ascontiguousarray(X, dtype=np.float64)
    y = np.ascontiguousarray(y, dtype=np.float64).reshape(-1)

    if X.shape[0] != y.shape[0]:
        raise ValueError("X and y have incompatible shapes for ridgeless fit.")
